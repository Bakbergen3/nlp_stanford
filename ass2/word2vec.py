...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    y_o_hat = softmax(outsideVectors @ centerWordVec)
    loss = - np.log(y_o_hat[outsideWordIdx])
    y = np.zeros(outsideVectors.shape[0])
    y[outsideWordIdx ] = 1
    gradCenterVec = outsideVectors.T @ (y_o_hat - y)
    gradOutsideVecs = (y_o_hat - y)[:, None] * centerWordVec[None, :]
    ### END YOUR CODE
