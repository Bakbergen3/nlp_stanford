...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        # x = torch.index_select(self.embeddings, 0, ]
        # Need to understand n_features to implement nn.Embedding manually
        embeddings = self.pretrained_embeddings.forward(w)
        x = embeddings.view(w.shape[0], self.embed_size * self.n_features)
    ...
    def forward(self, w):
        ...
        embed = embedding_lookup(w)
        hiddem = F.relu(embed @ self.embed_to_hidden_weight + self.embed_to_hidden_bias)
        dropout = self.dropout.forward(hidden)
        logits = dropout @ self.hidden_to_logits_weight + self.hidden_to_logits_bias
